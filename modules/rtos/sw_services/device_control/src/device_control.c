...
int resource_table_add(device_control_t *ctx, const control_resid_t resources[],
                       size_t num_resources, uint8_t servicer);

int resource_table_search(device_control_t *ctx, control_resid_t resid,
                          uint8_t *servicer);

control_ret_t device_control_servicer_cmd_recv(
    device_control_servicer_t *ctx,
    ...
    void *app_data, unsigned timeout) {
  ...
  if (status == RTOS_OSAL_SUCCESS) {
    ...
    if (IS_CONTROL_CMD_READ(c_ptr->cmd)) {
      ret = read_cmd_cb(c_ptr->resid, c_ptr->cmd, c_ptr->payload,
                        c_ptr->payload_len, app_data);
    } else {
      ret = write_cmd_cb(c_ptr->resid, c_ptr->cmd, c_ptr->payload,
                         c_ptr->payload_len, app_data);
    }

    if (device_control_ctx->resource_table != NULL) {
      status =
          rtos_osal_queue_send(&device_control_ctx->gateway_queue, &ret,
                               0); /* This should not block. As long as
                                      everything is working as designed, this
                                      queue will always be empty here. */
      xassert(status == RTOS_OSAL_SUCCESS);
    } else {
      /* gateway is on another tile */

      if (IS_CONTROL_CMD_READ(c_ptr->cmd)) {
        rtos_intertile_tx_len(device_control_ctx->host_intertile,
                              device_control_ctx->intertile_port,
                              sizeof(ret) + c_ptr->payload_len);
        rtos_intertile_tx_data(device_control_ctx->host_intertile, &ret,
                               sizeof(ret));
        rtos_intertile_tx_data(device_control_ctx->host_intertile,
                               c_ptr->payload, c_ptr->payload_len);
        /*
        ...
        rtos_intertile_tx(device_control_ctx->host_intertile,
                          device_control_ctx->intertile_port, &ret,
                          sizeof(ret));
      }
...
static void device_control_client_thread(device_control_t *ctx) {
  ...
  for (;;) {
    msg_length = rtos_intertile_rx(ctx->host_intertile, ctx->intertile_port,
                                   (void **)&c_ptr, RTOS_OSAL_WAIT_FOREVER);
...
static control_ret_t special_read_command(device_control_t *ctx,
                                          control_cmd_t cmd, uint8_t payload[],
...
static control_ret_t do_command(device_control_t *ctx, uint8_t servicer,
  ...
  if (resid == CONTROL_SPECIAL_RESID) {
    ...
    rtos_intertile_t *intertile_ctx =
        ctx->servicer_table[servicer].intertile_ctx;
    rtos_osal_queue_t *queue = ctx->servicer_table[servicer].queue;

    cmd_to_servicer_t c = {.dev_ctrl_ctx = ctx,
                           .queue = queue,
                           .resid = resid,
                           .cmd = cmd,
    ...
    if (intertile_ctx == NULL) { /* on tile case */
      ...
      rtos_osal_queue_receive(&ctx->gateway_queue, &ret,
      ...
      xfer_len = rtos_intertile_rx_len(intertile_ctx, ctx->intertile_port,
                                       RTOS_OSAL_WAIT_FOREVER);
...
control_ret_t device_control_payload_transfer(device_control_t *ctx,
  ...
  if (resource_table_search(ctx, requested_resid, &servicer) == 0) {
    if ((direction == CONTROL_DEVICE_TO_HOST &&
         IS_CONTROL_CMD_READ(requested_cmd)) ||
        (direction == CONTROL_HOST_TO_DEVICE &&
         !IS_CONTROL_CMD_READ(requested_cmd))) {
      if (requested_payload_len <= *buf_size) {
        ret = do_command(ctx, servicer, requested_resid, requested_cmd,
                         payload_buf, requested_payload_len);
        if (direction == CONTROL_DEVICE_TO_HOST) {
...
control_ret_t device_control_servicer_register(
    ...
    size_t num_resources) {
  const size_t len =
      sizeof(servicer_init_data_t) + sizeof(control_resid_t) * num_resources;
  ...
  for (int i = 0; i < device_control_ctx_count; i++) {
    ...
    memcpy(init_data->resources, resources,
           sizeof(control_resid_t) * num_resources);

    if (device_control_ctx[i]->resource_table != NULL) {
      rtos_osal_queue_send(&device_control_ctx[i]->gateway_queue, &init_data,
                           RTOS_OSAL_WAIT_FOREVER);

    } else {
      /* Resource table is NULL on client tiles */

      rtos_intertile_tx(device_control_ctx[i]->host_intertile,
                        device_control_ctx[i]->intertile_port, init_data, len);
...
static int servicer_register(device_control_t *ctx,
  ...
  ret = resource_table_add(ctx, init_cmd->resources, init_cmd->num_resources,
                           servicer_index);
...
control_ret_t device_control_resources_register(device_control_t *ctx,
  ...
  ctx->servicer_table =
      rtos_osal_malloc(servicer_count * sizeof(*ctx->servicer_table));

  start_time = rtos_osal_tick_get();
  while (registered_count < servicer_count && ret == 0 &&
         rtos_osal_tick_get() - start_time < timeout) {
    if (rtos_osal_queue_receive(&ctx->gateway_queue, &init_cmd, 1) ==
        RTOS_OSAL_SUCCESS) {
    ...
    for (int i = 0; i < ctx->intertile_count &&
                    registered_count < servicer_count && ret == 0;
         i++) {
      if (rtos_intertile_rx(ctx->client_intertile[i], ctx->intertile_port,
                            (void **)&init_cmd, 1) != 0) {
        ret = servicer_register(ctx, init_cmd, ctx->client_intertile[i],
                                registered_count);
        if (ret == 0) {
          registered_count++;
        }
...
control_ret_t device_control_start(device_control_t *ctx,
  ...
  if (ctx->resource_table == NULL) {
    ...
    status = rtos_osal_thread_create(
        NULL, "dc_client",
        (rtos_osal_entry_function_t)device_control_client_thread, ctx,
        RTOS_THREAD_STACK_SIZE(device_control_client_thread), priority);
