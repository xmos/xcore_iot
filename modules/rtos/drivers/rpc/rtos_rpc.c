...
int rpc_request_marshall_va(uint8_t **msg, int fcode,
  ...
  msg_length = sizeof(int) + /* Space for the function code */
               ...
               sizeof(rpc_param_desc_t) *
                   param_count +   /* Space for each parameter descriptor */
...
int rpc_request_marshall(uint8_t **msg, int fcode,
...
int rpc_response_marshall_va(uint8_t **msg, const rpc_msg_t *rpc_msg,
                             va_list ap) {
  int param_total_length = 0;
  int i;
  int msg_length;
...
void rpc_response_unmarshall_va(const rpc_msg_t *rpc_msg,
                                const rpc_param_desc_t param_desc[],
                                va_list ap) {
  int i;
  uint8_t *params_ptr = rpc_msg->params;
...
void rpc_response_unmarshall(const rpc_msg_t *rpc_msg,
                             const rpc_param_desc_t param_desc[], ...) {
  va_list ap;
...
void rpc_client_call_generic(rtos_intertile_t *intertile_ctx, uint8_t port,
                             int fcode, const rpc_param_desc_t param_desc[],
                             ...) {
  uint8_t *req_msg;
  uint8_t *resp_msg;
  rpc_msg_t rpc_msg;
  ...
  va_copy(ap, ap_init);
  msg_length = rpc_request_marshall_va(&req_msg, fcode, param_desc, ap);
  va_end(ap);
  ...
  msg_length = rtos_intertile_rx(intertile_ctx, port, (void **)&resp_msg,
