...
/**
 * \addtogroup rtos_i2s_master_driver
 * @{
 */

/**
 * \defgroup rtos_i2s_master_driver_rpc
 *
 * The functions for setting up RPC with an RTOS I2S master driver instance.
 * @{
 */

/**
 * Initializes an RTOS I2S master driver instance on a client tile.
 ...
 * instead of rtos_i2s_master_init(). The host tile that owns the actual
 * instance must simultaneously call rtos_i2s_master_rpc_host_init().
 *
 ...
 *                           the same scope as \p i2s_master_ctx.
...
/**
 * Performs additional initialization on a I2S master driver instance to
 * allow client tiles to use the I2S master driver instance. Each client
 * tile that will use this instance must simultaneously call
 * rtos_i2s_master_rpc_client_init().
 *
 * \param i2s_master_ctx       A pointer to the I2S master driver instance to share with clients.
 * \param rpc_config           A pointer to an RPC config struct. This must have
 *                             the same scope as \p i2s_master_ctx.
...
/**
 * Configures the RPC for a I2S master driver instance. This must be called
 * by both the host tile and all client tiles.
 *
 * On the client tiles this must be called after calling rtos_i2s_master_rpc_client_init().
 * After calling this, the client tile may immediately begin to call the core I2S master
 * functions on this driver instance. It does not need to wait for the host to call
 * rtos_i2s_master_start().
 *
 * On the host tile this must be called both after calling rtos_i2s_master_rpc_host_init()
 * and before calling rtos_i2s_master_start().
 *
 * \param i2s_master_ctx     A pointer to the I2S master driver instance to configure the RPC for.
