...
#define TEST_DEVICE_ASYNC    0
#define TEST_DEVICE_SYNC     0
#define TEST_ASYNC_SPI  0
#define TEST_SYNC_SPI   1
        ...
        client spi_master_if ?i_spi)
{
    ...
    while (1)
    {
        if (peripheral != NULL && *peripheral != NULL) {
            if( transfer == 0 )
            {
                data_len = soc_peripheral_rx_dma_direct_xfer(*peripheral, tx_buf, SPICONF_TX_BUFSIZE);
                ...
                if (data_len > 0) {
                    ...
                    {
                        rx_buf[i] = i_spi.transfer8(tx_buf[i]);
                    }
                }
            }
        }
...
void spi_dev(
        ...
        clock ?clk1)
{
#if ( TEST_SYNC_SPI || TEST_DEVICE_SYNC )
    spi_master_if i_spi[1];
#endif
#if ( TEST_ASYNC_SPI || TEST_DEVICE_ASYNC )
