...
def test_model_path():
    with XCOREInterpreter(model_path=BUILTIN_OPERATORS_TEST_MODEL) as interpreter:
        assert interpreter
...
def test_callback():
    ...
    with XCOREInterpreter(model_content=model_content) as interpreter:
        ...
        interpreter.set_tensor(input_tensor_details["index"], input_tensor)
        interpreter.invoke(
            preinvoke_callback=preinvoke_callback,
            postinvoke_callback=postinvoke_callback,
        )
