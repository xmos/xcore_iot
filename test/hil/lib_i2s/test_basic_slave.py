...
def test_basic_slave(build, capfd, nightly, num_in, num_out):
    test_level = "nightly" if nightly else "smoke"
    id_string = f"{test_level}_{num_in}_{num_out}"
    binary = f'i2s_slave_test/bin/{id_string}/i2s_slave_test_{id_string}.xe'
    ...
    tester = px.testers.PytestComparisonTester('expected/slave_test.expect',
                                            regexp = True,
                                            ordered = True,
                                            ignore = ["CONFIG:.*?"])
