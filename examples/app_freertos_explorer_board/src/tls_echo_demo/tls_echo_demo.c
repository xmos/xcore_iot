...
static void tls_echo_receiver( void *arg )
{
    		...
    		!= 0 )
    {
        ...
        configASSERT(0); /* TODO: cleanup and delete task */
    }
...
static void tls_echo( void *arg )
{
    ...
    for( ;; )
    {
    	if( handle->connected == pdFALSE )
    	{
	    	...
	    	if(	FreeRTOS_connect( xConnectedSocket, &xBindAddress, sizeof( xBindAddress ) ) == 0 )
	    	{
