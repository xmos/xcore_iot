...
port p_rst_shared  = PORT_SHARED_RESET;
#define I2S_CHANS_DAC 4

static void initCS43L21(client i2c_master_if i2c)
{
    i2c_regop_res_t res;
    ...
    for (int i = CS43L21_I2C_ADDR; i < CS43L21_I2C_ADDR + (I2S_CHANS_DAC / 2); i++) {
         ...
         */
        res = i2c.write_reg(i, CS43L21_REG_POWER_CTL, 0x01);
        xassert(res == I2C_REGOP_SUCCESS);
        /* I2S Mode
         * |7:reserved|6:M/S|5..3:fmt|2..0:reserved|
         */
        // I2S mode, up to 24-bit data
        res = i2c.write_reg(i, CS43L21_REG_IFACE_CTL, 0x08);
        xassert(res == I2C_REGOP_SUCCESS);
        /* Speed Control
         * |7:AUTO|6..5:SPEED|4:tristate|3..1:reserved|0:MCLKDIV2|
         */
        res = i2c.write_reg(i, CS43L21_REG_SPEED_CTL, 0b10000001);
        xassert(res == I2C_REGOP_SUCCESS);
        /* DAC Output Control
         * |7..5:ampgain|4:DAC_SNGVOL|3:INV_PCMB|2:INV_PCMA|1:DACB_MUTE|0:DACA_MUTE|
         */
        res = i2c.write_reg(i, CS43L21_REG_DAC_OUT_CTL, 0b00000000);
        xassert(res == I2C_REGOP_SUCCESS);
        /* DAC Control (disable DSP)
         * |7..6:DATA_SEL|5:FREEZE|4:reserved|3:DEEMPH|2:AMUTE|1::0:DAC_SZC|
         */
        res = i2c.write_reg(i, CS43L21_REG_DAC_CTL, 0x00);
        xassert(res == I2C_REGOP_SUCCESS);
        /* Power control (turn DAC on)
         * |7:reserved|6:PDN_DACB|5:PDN_DACA|4..1:reserved|0:PDN|
         */
        res = i2c.write_reg(i, CS43L21_REG_POWER_CTL, 0x00);
        xassert(res == I2C_REGOP_SUCCESS);
    }
}

void pll_init(client i2c_master_if i2c)
{
...
void audio_hw_config(client i2c_master_if i_i2c)
{
    p_rst_shared <: 0xF;
