...
void hotdog_not_hotdog_resolver_get(void **v_resolver)
{
  // Set up op resolver
  ...
  if (resolver == nullptr) {
    ...
    resolver->AddCustom(tflite::ops::micro::xcore::AvgPool2D_Global_OpCode, tflite::ops::micro::xcore::Register_AvgPool2D_Global());
...
void hotdog_not_hotdog_profiler_get(void **v_profiler) {
  if (profiler == nullptr) {
    // Set up profiling
...
void hotdog_not_hotdog_profiler_reset() {
  if (profiler) {
...
void hotdog_not_hotdog_profiler_durations_get(uint32_t *count, const uint32_t **durations) {
  if (profiler) {
...
void hotdog_not_hotdog_model_runner_create(model_runner_t *ctx, void *buffer) {
  ...
  ctx->resolver_get_fun = &hotdog_not_hotdog_resolver_get;
#ifndef NDEBUG
  ctx->profiler_get_fun = &hotdog_not_hotdog_profiler_get;
  ctx->profiler_reset_fun = &hotdog_not_hotdog_profiler_reset;
  ctx->profiler_durations_get_fun = &hotdog_not_hotdog_profiler_durations_get;
#endif
}