...
int get_wav_header_details(FILE *input_file, wav_header *s, unsigned *header_size){
  //Assume file is already open here. First rewind.
  if(input_file == NULL)
  {
    printf("Error: file is not initialised");
    return 1;
  }
  fseek(input_file, 0, SEEK_SET);
  //read riff header section (12 bytes)
  //file_read(input_file, (uint8_t*)(&s->riff_header[0]), RIFF_SECTION_SIZE);
  fread(s->riff_header, RIFF_SECTION_SIZE, 1, input_file);
  ...
  fread(s->fmt_header, FMT_SUBCHUNK_MIN_SIZE, 1, input_file);
  ...
  if(s->audio_format == (short)0xfffe)
  {
    ...
    fseek(input_file, fmt_subchunk_remaining_size - EXTENDED_FMT_GUID_SIZE, SEEK_CUR);
    //The first 2 bytes of GUID is the audio_format.
    //file_read(input_file, (uint8_t *)&s->audio_format, sizeof(s->audio_format));
    fread(&s->audio_format, sizeof(s->audio_format), 1, input_file);
    //skip the rest of GUID
    fseek(input_file, EXTENDED_FMT_GUID_SIZE - sizeof(s->audio_format), SEEK_CUR);
  }
  else
  {
    //go to the end of fmt subchunk
    fseek(input_file, fmt_subchunk_remaining_size, SEEK_CUR);
  }
  if(s->audio_format != 1)
  {
  ...
  fread(s->data_header, sizeof(s->data_header), 1, input_file);
  ...
  if(memcmp(s->data_header, "fact", sizeof(s->data_header)) == 0)
  {
    ...
    fread(&chunksize, sizeof(s->data_bytes), 1, input_file);
    fseek(input_file, chunksize, SEEK_CUR);
    //file_read(input_file, (uint8_t*)(&s->data_header[0]), sizeof(s->data_header));
    fread(s->data_header, sizeof(s->data_header), 1, input_file);
  }
  ...
  fread(&s->data_bytes, sizeof(s->data_bytes), 1, input_file);
  *header_size = ftell(input_file); //total file size should be header_size + data_bytes
