    ...
    for (int i = 0; i < 10; i++) {
      ...
      rtos_printf("\tSecurity: %s\n",
                  scan_result->xSecurity == eWiFiSecurityOpen
                      ? "open"
                      : scan_result->xSecurity == eWiFiSecurityWEP ? "WEP"
                                                                   : "WPA");
...
static void wifi_test_task(wifi_test_devs_t *wifi_devs) {
  ...
  const uint8_t ucIPAddress[4] = {
      IPconfig_IP_ADDR_OCTET_0, IPconfig_IP_ADDR_OCTET_1,
      IPconfig_IP_ADDR_OCTET_2, IPconfig_IP_ADDR_OCTET_3};
  const uint8_t ucNetMask[4] = {
      IPconfig_NET_MASK_OCTET_0, IPconfig_NET_MASK_OCTET_1,
      IPconfig_NET_MASK_OCTET_2, IPconfig_NET_MASK_OCTET_3};
  const uint8_t ucGatewayAddress[4] = {
      IPconfig_GATEWAY_OCTET_0, IPconfig_GATEWAY_OCTET_1,
      IPconfig_GATEWAY_OCTET_2, IPconfig_GATEWAY_OCTET_3};
  const uint8_t ucDNSServerAddress[4] = {
      IPconfig_DNS_SERVER_OCTET_0, IPconfig_DNS_SERVER_OCTET_1,
      IPconfig_DNS_SERVER_OCTET_2, IPconfig_DNS_SERVER_OCTET_3};
  ...
  rtos_printf("Completed WiFi test on tile %d\n", THIS_XCORE_TILE);

  vTaskDelete(NULL);
}

void wifi_test_start(rtos_spi_master_device_t *wifi_device_ctx,
                     rtos_gpio_t *gpio_ctx) {
  ...
  xTaskCreate((TaskFunction_t)wifi_test_task, "wifi_test_task",
              RTOS_THREAD_STACK_SIZE(wifi_test_task), &wifi_devs,
              configMAX_PRIORITIES / 2 - 1, NULL);
}
