...
__attribute__((section(".SwMem_data"))) uint8_t sw_mem_array[32 * 10];
...
void rtos_swmem_write_request(unsigned offset, uint32_t dirty_mask,
                              const uint32_t *buf) {
...
void swmem_test_run(void) {
  ...
  rtos_printf("Completed SwMem test on tile %d\n", THIS_XCORE_TILE);
}
