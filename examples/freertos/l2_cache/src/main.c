...
static int l2_cache_buffer[RTOS_L2_CACHE_BUFFER_WORDS_DIRECT_MAP];
#else
__attribute__((aligned(8)))
static int l2_cache_buffer[RTOS_L2_CACHE_BUFFER_WORDS_TWO_WAY];
#endif /* DIRECT_MAP */

L2_CACHE_SWMEM_READ_FN
void rtos_flash_read_wrapper(
    void* dst_address,
    ...
    const unsigned bytes)
{
    ...
    int ret = -1;
    do {
        ret = rtos_qspi_flash_read_ll(qspi_flash_ctx,
                                    (uint8_t*) dst_address,
...
void app(void* args)
{
    ...
    while(1) {
        rtos_printf("Run benchmarks\n");

        //   sequential data access
        benchmark_sequential_data_throughput();

        //   sequential data with prefetch
        benchmark_sequential_data_throughput_with_prefetch();

        //   random data access
        benchmark_random_data_throughput();
