        ...
        QueueHandle_t input_queue)
{
    if( gpio_ctx != NULL )
    {
        ...
        xTaskCreate((TaskFunction_t) person_detect_app_task,
                    "person_detect_app",
                    RTOS_THREAD_STACK_SIZE(person_detect_app_task),
                    args,
                    priority,
                    NULL);
    } else {
        ...
        unsigned priority)
{
    ...
    xTaskCreate((TaskFunction_t) person_detect_task_runner,
                "person_detect",
                500,
                args,
                priority,
    ...
    xTaskCreate((TaskFunction_t) person_detect_runner_rx,
                "person_detect_rx",
                RTOS_THREAD_STACK_SIZE(person_detect_runner_rx),
                args,
                priority-1,
                NULL);
}
