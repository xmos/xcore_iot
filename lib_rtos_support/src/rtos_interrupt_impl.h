...
#define _DEFINE_RTOS_INTERRUPT_PERMITTED_DEF(root_function) \
    ...
    /*  - This wrapper function: _XCORE_C_STACK_ALIGN(3) + __xcore_interrupt_permitted_common.nstackwords  */ \
    ...
    .set   _XCORE_INTERRUPT_PERMITTED(root_function).nstackwords, _XCORE_STACK_ALIGN(3) + RTOS_SUPPORT_INTERRUPT_STACK_GROWTH + _kstack_words + __xcore_interrupt_permitted_common.nstackwords + root_function.nstackwords; \
...
#define _DEFINE_RTOS_INTERRUPT_PERMITTED(ret, root_function, ...) \
    asm(RTOS_STRINGIFY(_DEFINE_RTOS_INTERRUPT_PERMITTED_DEF(root_function))); \
